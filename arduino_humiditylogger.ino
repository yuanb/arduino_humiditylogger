...
unsigned long getNtpTime() {
  ...
  /************** NTP STUFF ***************/ 
  byte* packetBuffer = (byte *)malloc(NTP_PACKET_SIZE * sizeof(byte));
...
void sampleSensorTask()
{
  ...
  if ((millis() % lastIntervalTime) >= MEASURE_INTERVAL)    //Is it time for a new measurement?
  {
    ...
    Serial.print(F(" lastIntervalTime="));
    ...
    {        
      ...
      float temperature = 21.0; //replace with a thermometer reading if you have it
...
void serveClientTask() {
   ...
   if (client)
   {
      ...
      char* clientline = (char *)malloc( BUFSIZ * sizeof(char));
